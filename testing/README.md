...
### 1. 실제 사용자 10000명 부하테스트 실행
```bash
# testing 폴더로 이동
cd testing

# 실제 10000명 사용자 부하테스트 실행
./scripts/run_real_user_test.bat
```

### 2. 기본 부하테스트 실행 (CSV 기반)
```bash
# testing 폴더로 이동
cd testing
...
1. **10000명 사용자 데이터 생성**
   ...
   python create_cheongra_users.py --insert-db
   ```

2. **실제 API 엔드포인트 확인**
   - 로그인: `/api/v1/members/login`
   - 사용자 정보: `/api/v1/members/me`
   - 채팅방 입장: `/api/v1/chat/region-info`

### 테스트 시나리오
- **로그인 API 호출**: 실제 사용자 이메일/비밀번호로 로그인
- **JWT 토큰 획득**: 응답에서 토큰 추출 및 저장
- **사용자 정보 조회**: 로그인 후 사용자 정보 API 호출
- **채팅방 입장**: 청라1동 Region ID(1229) 채팅방 입장

### 테스트 설정
- **총 사용자**: 10000명 (실제 데이터베이스 사용자)
- **테스트 단계**: 4단계 (100 → 1000 → 10000명)
- **테스트 시간**: 약 3-4분
- **부하 패턴**: 단계적 증가 (현실적인 부하 시뮬레이션)
...
### 기대 결과
- **인덱스 효과 확인**: 로그인 시 `email` 인덱스 활용
- **복합 인덱스 효과**: 채팅방 입장 시 `region_id` 복합 인덱스 활용
- **실제 사용자 시뮬레이션**: 데이터베이스에 존재하는 10000명 사용자 대상 테스트
- **현실적인 성능 측정**: 실제 API 호출 패턴 시뮬레이션
