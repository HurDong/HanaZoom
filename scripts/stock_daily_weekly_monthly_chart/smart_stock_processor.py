...
"""
스마트 주식 데이터 처리 스크립트
스마트 수집된 CSV 파일들을 효율적으로 데이터베이스에 저장
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('smart_stock_processing.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SmartStockProcessor:
    def __init__(self, db_config):
        self.db_config = db_config
        self.connection = None
        self.cursor = None
        self.csv_dir = "./out_krx_smart"  # 스마트 수집된 CSV 파일들
    ...
    def test_db_connection(self):
        ...
        try:
            ...
            # 샘플 데이터로 마지막 날짜 조회 테스트
            test_query = "SELECT MAX(trade_date) FROM stock_daily_prices LIMIT 1"
            self.cursor.execute(test_query)
            result = self.cursor.fetchone()
            if result and result[0]:
                logger.info(f"📅 DB에서 확인된 마지막 일별 데이터 날짜: {result[0]}")
            else:
                logger.info("📅 DB에 일별 데이터가 없습니다")
    ...
    def bulk_insert_daily_data(self, stock_code, df):
        """일별 데이터 벌크 삽입 (ON DUPLICATE KEY UPDATE 사용)"""
        if df.empty:
            return 0
    ...
    def bulk_insert_weekly_data(self, stock_code, df):
        """주별 데이터 벌크 삽입 (ON DUPLICATE KEY UPDATE 사용)"""
        if df.empty:
            return 0
    ...
    def bulk_insert_monthly_data(self, stock_code, df):
        """월별 데이터 벌크 삽입 (ON DUPLICATE KEY UPDATE 사용)"""
        if df.empty:
            return 0
        ...
        try:
            ...
            if os.path.exists(daily_file):
                ...
                if df_daily is not None:
                    ...
                    logger.info(f"📊 {stock_code} 일별 데이터 {result['daily_inserted']}건 처리 완료")
            ...
            if os.path.exists(weekly_file):
                ...
                if df_weekly is not None:
                    ...
                    logger.info(f"📈 {stock_code} 주별 데이터 {result['weekly_inserted']}건 처리 완료")
            ...
            if os.path.exists(monthly_file):
                ...
                if df_monthly is not None:
                    ...
                    logger.info(f"📅 {stock_code} 월별 데이터 {result['monthly_inserted']}건 처리 완료")
    ...
    def process_all_data_smart(self, max_workers=None):
        """스마트 데이터 처리 - 멀티스레딩으로 효율적 처리"""
        start_time = time.time()
        
        logger.info("🚀 스마트 주식 데이터 처리 시작!")
        
        # CSV 파일 목록 조회
        csv_files = self.get_csv_files()
        
        if not csv_files:
            logger.warning("⚠️ 처리할 CSV 파일이 없습니다.")
            return []
        ...
        if max_workers is None:
            max_workers = min(8, len(all_stocks))  # 최대 8개 스레드
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # 진행률 표시
            with tqdm(total=len(all_stocks), desc="🚀 스마트 데이터 처리 진행률") as pbar:
        ...
        total_data = total_daily + total_weekly + total_monthly
        
        # 성공한 종목 수 계산
        successful_stocks = len([r for r in total_results if not r['errors']])
        failed_stocks = len([r for r in total_results if r['errors']])
        
        logger.info("=" * 60)
        logger.info("🏁 스마트 데이터 처리 완료!")
        logger.info(f"⏱️  총 처리 시간: {processing_time:.2f}초")
        logger.info(f"⚡ 스레드 워커: {max_workers}개")
        logger.info(f"📊 처리된 종목: {len(all_stocks)}개 (성공: {successful_stocks}개, 실패: {failed_stocks}개)")
        logger.info(f"📈 일별 데이터: {total_daily:,}건")
        logger.info(f"📊 주별 데이터: {total_weekly:,}건")
        logger.info(f"📅 월별 데이터: {total_monthly:,}건")
        logger.info(f"📊 총 저장 데이터: {total_data:,}건")
...
def main():
    ...
    # 스마트 프로세서 생성
    processor = SmartStockProcessor(db_config)
    
    # 데이터베이스 연결 및 테스트
    logger.info("🔍 데이터베이스 연결 및 테스트 시작...")
    if not processor.connect_db():
        logger.error("❌ 데이터베이스 연결 실패 - 스크립트를 중단합니다")
        logger.error("❌ 불필요한 처리를 방지하기 위해 스크립트를 종료합니다")
        return
    
    logger.info("✅ 데이터베이스 연결 및 테스트 완료 - 스마트 데이터 처리를 시작합니다")
    
    try:
        # 스마트 데이터 처리 실행
        results = processor.process_all_data_smart()
        ...
        logger.info("💾 처리 결과가 smart_processing_results.json 파일에 저장되었습니다.")
